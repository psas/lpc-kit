...
#
# USAGE: kftdlib.sh (-i | -u) [-h]
#            The arguments to use are:
#           -h: long help message
#           -i: Attempt an install of openOCD libs
#           -u: Attempt an uninstall of openOCD libs
#
# $Id$
#
#
...
###########################
# lpc environment
# infomsg "Setting up local env"
# --$HOME/Tools/setenv_$kocd_board.sh

# backup old openocd lib...
newdir=$LIB.$DAY$MONTH$YEAR.$TIME
if [ -d $LIB ]; then
  command="cp -r $LIB $newdir"
  excmd "$command"
fi
...
if [ $KERN_MINOR_VERSION -eq 6 ]; then
	...
	command="sudo sh -c \"echo none /proc/bus/usb usbfs defaults,mode=0666 0 0  >> /etc/fstab\" "
        excmd "$command"
    elif    [ $check_fstab -eq 1 ] && [ "$undo" -eq 1 ]   ; then
	...
	infomsg "Strange setup...already undone?: install: $install undo: $undo check_fstab: $check_fstab"
#	exit $FAIL	
    fi
...
if [ $check_ldso -eq 0 ] && [ "$install" -eq 1 ]; then 
    ...
    excmd "$command"
elif  [ $check_ldso -eq 1 ] && [ "$undo" -eq 1 ]; then
    ...
    infomsg "Strange setup...already undone?: install: $install undo: $undo check_ldso: $check_ldso"
  #  exit $FAIL	
fi
